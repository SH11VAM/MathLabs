...
const VisualOne = () => {
  ...
  const handleInputBlur = () => {
    if (isEditingFirst) {
      const newValue = tempValue === "" ? 0 : parseInt(tempValue, 10);
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-sky-blue">
      ...
      <Cloud className="absolute top-16 left-1/2 transform -translate-x-1/2" size="md" />
      
    
      
      
      {/* Butterflies */}
      <Butterfly className="absolute top-32 left-40" color="#9B59B6" /> {/* Purple */}
      <Butterfly className="absolute top-32 left-56" color="#E74C3C" /> {/* Red */}
      <Butterfly className="absolute top-5 left-80" color="#3498DB" /> {/* Blue */}
          ...
          <div className="absolute top-0 left-0 w-full flex justify-around">
            {[...Array(20)].map((_, i) => (
              <div 
                ...
                style={{ transform: i % 2 === 0 ? 'rotate(-5deg)' : 'rotate(5deg)' }}
              ></div>
            ))}
          </div>
        </div>
